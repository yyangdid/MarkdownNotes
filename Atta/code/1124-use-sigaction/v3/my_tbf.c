...
// 因为不使用 signal(2) 了, 所以用不到 sighandler_t 了
// 保存 SIGALRM 原本的动作
...
//   }
// }
static void alarm_action(int s, siginfo_t *info_ptr, void *unused) {
  // 如果信号是从内核来的就响应,否则不响应
  // alarm(1);
  if (info_ptr->si_code != SI_KERNEL)
    return;
