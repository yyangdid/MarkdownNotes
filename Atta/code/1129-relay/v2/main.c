 ...
 * @return 较大的文件描述符.
 */
static int max_fd(int fd1, int fd2) {
  if (fd1 > fd2)
    return fd1;
  return fd2;
}
...
static void relay(int fd1, int fd2) {
  ...
  // }
  fd_set read_set, write_set; // 文件描述符集合, 读集和写集
  while (fsm_r1_w2.state != STATE_T || fsm_r2_w1.state != STATE_T) {
    // 布置监视任务
    FD_ZERO(&read_set); // 将读集置为空集
    FD_ZERO(&write_set);
    if (fsm_r1_w2.state == STATE_R)        // 若状态机 1 的状态为读
      FD_SET(fsm_r1_w2.src_fd, &read_set); // 将状态机 1 的源文件描述符加入读集
    if (fsm_r1_w2.state == STATE_W)
      FD_SET(fsm_r1_w2.des_fd, &write_set);
    if (fsm_r2_w1.state == STATE_R)
      FD_SET(fsm_r2_w1.src_fd, &read_set);
    if (fsm_r2_w1.state == STATE_W)
      FD_SET(fsm_r2_w1.des_fd, &write_set);

    // 监视
    // 当状态机状态为读取态或写入态时
    if (fsm_r1_w2.state < STATE_AUTO || fsm_r2_w1.state < STATE_AUTO) {
      // 盲等, 直到感兴趣的时间发送(阻塞在 select(2), cpu 不再是满载状态了)
      if (select(max_fd(fd1, fd2) + 1, &read_set, &write_set, NULL, NULL) < 0) {
        // 当 select(2) 函数返回的时候，其参数中的集和就已经不是执行 select() 时的内容了，不能用
        if (errno == EINTR) // 假错, 被信号打断. select(2) 参数中的集和被清空了
          continue;
        perror("select(2)"); // 真错
        exit(EXIT_FAILURE);
      }
    ...
    // 查看监视结果
    // 根据监视结果来有条件的推动状态机
    if (FD_ISSET(fd1, &read_set) || FD_ISSET(fd2, &write_set) || fsm_r1_w2.state > STATE_AUTO)
      fsm_driver(&fsm_r1_w2); // 推动状态机
    if (FD_ISSET(fd2, &read_set) || FD_ISSET(fd1, &write_set) || fsm_r2_w1.state > STATE_AUTO)
      fsm_driver(&fsm_r2_w1);
  }
