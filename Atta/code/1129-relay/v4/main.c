...
static void relay(int fd1, int fd2) {
  ...
  int ep_fd = epoll_create(10); // 创建 epoll 实例
  if (ep_fd < 0) {              // 创建 epoll 失败
    perror("epoll_create()");
    exit(1);
  }
  ...
  ev.events  = 0; // 位图置零
  ev.data.fd = fd1;
  epoll_ctl(ep_fd, EPOLL_CTL_ADD, fd1, &ev); // 添加文件描述符到 epoll

  ev.events  = 0; // 位图置零
  ev.data.fd = fd2;
  epoll_ctl(ep_fd, EPOLL_CTL_ADD, fd2, &ev);
  while (fsm_r1_w2.state != STATE_T || fsm_r2_w1.state != STATE_T) {
    ...
    if (fsm_r2_w1.state == STATE_W) // 若状态机 2 的状态为写，那 fd1 可写
      ev.events |= EPOLLOUT;
    epoll_ctl(ep_fd, EPOLL_CTL_MOD, fd1, &ev); // 进行文件描述符的添加、删除、修改等
    ...
    if (fsm_r2_w1.state == STATE_R) // 若状态机 2 的状态为读，那 fd2 可读
      ev.events |= EPOLLIN;
    epoll_ctl(ep_fd, EPOLL_CTL_MOD, fd2, &ev); // 进行文件描述符的添加、删除、修改等

    // 监视
    // 当状态机状态为读取态或写入态时
    if (fsm_r1_w2.state < STATE_AUTO || fsm_r2_w1.state < STATE_AUTO) {
      while (epoll_wait(ep_fd, &ev, 1, -1) < 0) { // 进行监视
